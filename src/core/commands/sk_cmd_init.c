...
void sk_cmd_init_fn(struct sk_ctx *ctx)
{
    ...
    {
        vx_printf(SK_ANSI_GRAY "[TIP]: You can now 'cd %s'\n" SK_ANSI_RESET, rpath);
        vx_printf(SK_ANSI_GRAY "[TIP]: To make a simple main.c: 'sk --main-c'\n" SK_ANSI_RESET);
        vx_printf(SK_ANSI_GRAY "[TIP]: Commands and option flags can be chained eg: 'sk --main-c "
                               "strike --profile status surge'\n" SK_ANSI_RESET);
        vx_printf(SK_ANSI_GRAY "[TIP]: To remove tips: `sk config --show-tips=0`" SK_ANSI_RESET);
    }
}
