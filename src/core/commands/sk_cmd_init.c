...
// NOTE: keep an eye how it loads values
...
{
    if (vx_fs_exists(custom_path) != VX_OK)
    {
        return;
    }

    const char *cc_name = custom_path;  // eg: gcc

    for (const char *p = custom_path; *p; p++)
    {
    ...
    sk_meta_write_cc_entry(custom_path, cc_name, rpath);
}
